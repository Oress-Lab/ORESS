document.addEventListener('DOMContentLoaded', function() {
    ...
    // Expose course selection functions
    window.courseSelectionModule = {
        addToSelected: (crn) => {
        ...
        isSelected: (crn) => {
            return selectedCourses.has(crn);
        }
    };
    ...
    if (savedSelected) {
        try {
            ...
            selectedCourses = new Set(selectedArray);
            
            // Update checkboxes for selected courses
            selectedArray.forEach(crn => {
                document.querySelectorAll(`[data-course-crn="${crn}"] input[type="checkbox"]`).forEach(cb => {
                    cb.checked = true;
                });
            });
    ...
    function createCourseElement(course, isFavorite = false) {
        ...
        // Check if the course is actually in the timetable
        const isInTimetable = document.querySelector(`.course-block[data-course-crn="${course.crn}"]`) !== null;
    ...
    searchForm.addEventListener('submit', async function(e) {
        ...
        try {
            // Get current session values from the form
            const campusSelect = document.querySelector('select[name="campus"]');
            const semesterYearSelect = document.querySelector('select[name="semester_year"]');
            
            const campus = campusSelect ? campusSelect.value : 'main';
            const semesterYear = semesterYearSelect ? semesterYearSelect.value : 'fall_2024';
            
            // Split semester_year into semester and year
            const parts = semesterYear.split('_');
            const semester = parts[0] ? parts[0].charAt(0).toUpperCase() + parts[0].slice(1) : 'Fall';
