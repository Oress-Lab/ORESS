document.addEventListener('DOMContentLoaded', function() {
    ...
    function addCourseToTimetable(courseElement) {
        ...
        daysArray.forEach((dayString, index) => {
            ...
            if (dayString === 'TTh') {
                days = ['T', 'H'];
            } else if (dayString === 'M-F') {
                // Handle M-F pattern (summer courses) - add blocks for all weekdays
                days = ['M', 'T', 'W', 'H', 'F'];
            } else {
    ...
    function calculateLeftPosition(day) {
        // Day column widths and offsets
        const columnWidth = 100;
        const dayColumnOffsets = {
        ...
        // Get the base position for the day
        const basePosition = dayColumnOffsets[day] || 65;
        
        // Calculate the centered position (column center minus half of block width)
        return basePosition;
    }
    ...
    function extractDays(courseTime) {
        ...
        // Check for "M-F" pattern (summer courses)
        if (courseTime.includes('M-F')) {
            return ['M-F']; // Return as a special pattern to be processed later
        }
    ...
    function parseTimeSlots(courseTime) {
        ...
        timeComponents.forEach(component => {
            ...
            // Handle M-F pattern for summer courses
            if (days === 'M-F') {
                ...
                // Special handling for TTh pattern
                ['T', 'H'].forEach(day => {
                    slots.push({
                        day,
                        start: convertTimeToMinutes(startTime),
    ...
    async function downloadTimetablePDF() {
        const timetableElement = document.getElementById('schTimes');
        if (!timetableElement) {
        ...
        try {
            // Collect all course blocks
            const courseBlocks = document.querySelectorAll('.course-block');
            const courses = Array.from(courseBlocks).map(block => ({
                code: block.getAttribute('data-course-code'),
                name: block.getAttribute('data-course-name'),
                time: block.getAttribute('data-course-time'),
                room: block.getAttribute('data-course-room'),
                crn: block.getAttribute('data-course-crn')
            }));

            // Send request to backend
            const response = await fetch('/download_timetable', {
                method: 'POST',
                headers: {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (downloadButton) {
            downloadButton.addEventListener('click', downloadTimetablePDF);
        }
    });
