document.addEventListener('DOMContentLoaded', function() {
    ...
    function addCourseToTimetable(courseElement) {
        ...
        daysArray.forEach((dayString, index) => {
            ...
            days.forEach(day => {
                ...
                courseBlock.setAttribute('data-crn', crn);
                ...
                // Add click event listener to remove course
                courseBlock.addEventListener('click', (e) => {
                    if (confirm(`Remove ${courseCode} from timetable?`)) {
                        removeCourseFromTimetable(crn);
                        // Also uncheck the course in the list
                        const checkbox = document.querySelector(`input[type="checkbox"][data-crn="${crn}"]`);
                        if (checkbox) {
                            checkbox.checked = false;
                        }
                        // Remove from favorites if it exists there
                        const favorite = document.querySelector(`.favorite-course[data-crn="${crn}"]`);
                        if (favorite) {
                            favorite.remove();
                        }
                        // Update the star state
                        const star = document.querySelector(`[data-star-crn="${crn}"]`);
                        if (star) {
                            star.classList.remove('text-yellow-400');
                            star.classList.add('text-gray-400');
                        }
                        // Remove from favorites set
                        if (window.timetableModule.favorites) {
                            window.timetableModule.favorites.delete(crn);
                        }
                    }
                });
    ...
    function removeCourseFromTimetable(crn) {
        // Find blocks with matching CRN
        const blocks = document.querySelectorAll(`.course-block[data-crn="${crn}"]`);
        
        // Remove each block and its CRN from tracking
        ...
        if (checkbox) {
            checkbox.checked = false;
            // Also update the selected courses set if it exists
            if (window.timetableModule.selectedCourses) {
                window.timetableModule.selectedCourses.delete(crn);
            }
        }
    ...
    function toggleCourseSelection(checkbox) {
        ...
        if (checkbox.checked) {
            ...
            // If adding fails (due to overlap or other issues), uncheck the checkbox
            if (!success) {
                checkbox.checked = false;
                showOverlapWarning();
            } else {
                // Save the updated timetable state
                window.savetbModule.saveTimetableState();
                
                // Add to favorites section when successfully added to timetable
                const favoritesUl = document.querySelector('#favorites ul');
                if (favoritesUl && !favoritesUl.querySelector(`[data-crn="${crn}"]`)) {
                    ...
                    ['data-crn', 'data-course-code', 'data-course-time', 'data-course-name', 'data-course-room'].forEach(attr => {
            ...
            const favoritesUl = document.querySelector('#favorites ul');
            if (favoritesUl) {
                const favoriteItem = favoritesUl.querySelector(`[data-crn="${crn}"]`);
                if (favoriteItem) {
                    favoriteItem.remove();
                }
            }
    ...
    window.timetableModule = {
        ...
        favorites: new Set(),
        toggleFavorite: function(course) {
            const crn = course.crn;
            const star = document.querySelector(`[data-star-crn="${crn}"]`);
            
            if (this.favorites.has(crn)) {
                this.favorites.delete(crn);
                // Remove from favorites section
                const favorite = document.querySelector(`.favorite-course[data-crn="${crn}"]`);
                if (favorite) {
                    favorite.remove();
                ...
                this.favorites.add(crn);
                // Add to favorites section
                const favoritesContainer = document.getElementById('favorites-container');
                if (favoritesContainer) {
                    const favoriteElement = document.createElement('div');
                    favoriteElement.classList.add('favorite-course', 'p-2', 'border', 'rounded', 'mb-2');
                    favoriteElement.setAttribute('data-crn', crn);
                    favoriteElement.innerHTML = `
                        <div class="flex justify-between items-center">
                            <span>${course.code} - ${course.name}</span>
                            <button class="text-red-500 hover:text-red-700" onclick="window.timetableModule.removeCourseFromTimetable('${crn}')">
    ...
    window.addEventListener('resize', () => {
        ...
        addedCoursesList.forEach(crn => {
            const courseElement = document.querySelector(`[data-crn="${crn}"]`);
            if (courseElement) {
                removeCourseFromTimetable(crn);
